...
const piecePrototype = {
  ...
  setPawnMovement() {
    ...
    this.Movement.addMovement(0, direction);
    if ((this.Color === "black" && this.Y === 1) || (this.Color === "white" && this.Y === this.BoardRows - 2)) {
      this.Movement.addMovement(0, direction * 2);
    }
  },
  ...
  setKnightMovement() {
    ...
    knightMoves.forEach(move => this.Movement.addMovement(...move));
  },
  ...
  setKingMovement() {
    ...
    kingMoves.forEach(move => this.Movement.addMovement(...move));
  },
  ...
  assignMovementForDirection(type) {
    ...
    directions[type].forEach(move => this.Movement.addMovement(...move));
  }
};
...
const boardPrototype = {
  ...
  calcMovement() {
    ...
    let { X: xi, Y: yi } = this.SelectedPiece;

    this.SelectedPiece.Movement.getMovements().forEach(([dx, dy]) => {
      if (dx === "n" || dy === "n") {
